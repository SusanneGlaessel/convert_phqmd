 ...
 ** Macro for creating the detector input from the PHSD-PHQMD output 
 **
...
struct PBaryon_cluster {
  ...
  Int_t fProcessId;
  Int_t fParentId;
  PBaryon_cluster(Int_t baryonId, Int_t PdgId, Int_t processId, Int_t parentId, TVector3 P, Float_t mass, Float_t energy, Float_t Ebin) : fBaryonId(baryonId), fPdgId(PdgId), fProcessId(processId), fParentId(parentId), fP(P), fMass(mass), fEnergy(energy), fEbin(Ebin) {};
};
				   ...
				   Bool_t ConvertAnti = kFALSE)
{
  ...
  for (int ievent=0;ievent<treeTmp->GetEntries();ievent++) {
	...
	for (Int_t baryonId : it_clId.second) {
	  ...
	  baryons_cluster.push_back(PBaryon_cluster(baryonId, baryon.fPdgId, hadron.fProcessId, hadron.fParentId, baryon.fP, baryon.fMass, baryon.fEnergy, baryon.fEbin));
	}
	if (nbary == 1) {
	  Int_t ibary  = 0;
	  output->cd();
	  uevent->AddParticle (index, baryons_cluster.at(ibary).fPdgId, 1, baryons_cluster.at(ibary).fParentId, -1, -1, baryons_cluster.at(ibary).fProcessId, child, baryons_cluster.at(ibary).fP.X(), baryons_cluster.at(ibary).fP.Y(), baryons_cluster.at(ibary).fP.Z(), baryons_cluster.at(ibary).fEnergy, 0, 0, 0, 0, 1);
	...
	if (nbary > 1) {
	  ...
	  if (pdgId != 99999 && Ebin/nbary < Ebin_max) {
	    ...
	    output->cd();
	    Int_t weight = 1;
	    uevent->AddParticle (index, pdgId, 1, clusterParentId, -1, -1, clusterProcessId, child, Px, Py, Pz, energy, 0, 0, 0, 0, weight);
	    
	    weight = 0;
	    for (int ibary=0;ibary<nbary;ibary++) // store single baryons with weight = 0
	      uevent->AddParticle (index, baryons_cluster.at(ibary).fPdgId, 1, baryons_cluster.at(ibary).fParentId, -1, -1, baryons_cluster.at(ibary).fProcessId, child, baryons_cluster.at(ibary).fP.X(), baryons_cluster.at(ibary).fP.Y(), baryons_cluster.at(ibary).fP.Z(), baryons_cluster.at(ibary).fEnergy, 0, 0, 0, 0, weight);
